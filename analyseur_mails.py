...
def sauvegarder_json(nouveaux):
    ...
    print(f"[JSON] mails.json : {len(tous)} mails ({len(a_ajouter)} nouveaux)")


def construire_html(devis_list):
...
<div style='background:#fff;border:1px solid #ddd;border-radius:8px;margin-bottom:20px;overflow:hidden'>
  <div style='background:#1a1a2e;padding:14px 20px'>
    <span style='color:#fff;font-weight:bold'>Demande #{i} — {cl.get('nom','?')}</span>
  ...
  <div style='padding:12px 20px;background:#f8f9fa;border-top:1px solid #eee;text-align:right'>
    <a href='https://devis-ineart.odoo.com/odoo/sales/new' style='background:#1a1a2e;color:#fff;padding:8px 18px;border-radius:6px;text-decoration:none;font-size:13px'>Creer dans Odoo</a>
  </div>
</div>"""
...
<div style='max-width:660px;margin:0 auto'>
  <div style='background:#1a1a2e;border-radius:8px 8px 0 0;padding:20px 24px;text-align:center'>
    ...
    <p style='color:#aaa;margin:6px 0 0;font-size:13px'>{date_str} · {len(devis_list)} demande(s)</p>
  </div>
...
def main(dry_run=False):
    ...
    for mail in mails_bruts:
        ...
        type_court = "D" if type_m == "DEMANDE_DEVIS" else "C" if type_m == "COMMANDE_VALIDEE" else "A"
