...
texte = re.sub(r”^`json\s*", "", texte) texte = re.sub(r"\s*`$”, “”, texte)
try:
...
for mail in mails_bruts:
    ...
    # PAUSE : analyse Claude désactivée temporairement
    # if est_expediteur_ignore(mail['de']):
    #     ...
    # analyse = analyser_mail(mail)
    # Pour l'instant on classe tout en AUTRE sans appeler l'API
    mail_dash = {
        "uid": mail["uid"], "de": mail["de"], "sujet": mail["sujet"],
        "date": datetime.now().strftime("%d/%m/%Y %H:%M"),
        "type": "A", "cf": 0,
        "cl": {"nom": "", "email": mail["de"], "ent": "", "tel": ""},
        "arts": [], "delai": "", "note": "⏸ Analyse IA en pause.",
    }
...
# PAUSE : envoi récap email désactivé temporairement
# if devis_detectes:
