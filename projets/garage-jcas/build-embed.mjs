 ...
 *
 *   node build-embed.mjs            → garage-jcas.embed.html
 *
...
let css = readFileSync(p("assets/css/style.css"), "utf8")
  .replace('url("../fonts/jcas-display.woff2")', `url("${dataUri(p("assets/fonts/jcas-display.woff2"), "font/woff2")}")`)
  .replace('url("../fonts/jcas-text.woff2")', `url("${dataUri(p("assets/fonts/jcas-text.woff2"), "font/woff2")}")`)
  .replace('url("../fonts/jcas-mono.woff2")', `url("${dataUri(p("assets/fonts/jcas-mono.woff2"), "font/woff2")}")`);
...
/* --- Images -------------------------------------------------------------- */
const images = {
  "assets/img/atelier.jpg": photo("atelier.jpg", 880, 76),
};
...
const sortie = p("garage-jcas.embed.html");
...
writeFileSync(p("garage-jcas.artifact.html"), fragment);
...
console.log(`garage-jcas.embed.html    — ${Math.round(statSync(sortie).size / 1024)} Ko`);
console.log(`garage-jcas.artifact.html — ${Math.round(statSync(p("garage-jcas.artifact.html")).size / 1024)} Ko`);
